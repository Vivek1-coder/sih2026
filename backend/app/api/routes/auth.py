...
from app.services.identity_service import (_normalize_aadhaar, _normalize_mobile, _normalize_abha, _normalize_email, _find_user_by_identifier)
...
def _auth_user(
    ...
    return AuthUser(
        ...
        is_mock=False,
        role=_claims_for(user, auth_method)["role"],
    )
...
def _claims_for(
    ...
    return {
        "role": user.staff_role if user.staff_role == "lab_assistant" else ("doctor" if Doctor.objects(pk=str(user.id)).first() else "patient"),
        "auth_method": auth_method,
        "display_name": user.full_name,
        "is_mock": False,
    }
...
def _issue_token_response(
    ...
    *, is_refresh: bool = False,
    ...
    if not is_refresh:
        audit(str(user.id), "patient_identified", metadata={"auth_method": auth_method})
    claims = _claims_for(
        user,
        auth_method,
    )
