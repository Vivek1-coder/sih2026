...
class SummaryService:
    ...
    def generate(
        ...
        if session.visit_status == "abandoned":
            raise HTTPException(409, "This visit was abandoned")
        existing = ClinicalHistorySummary.objects(interview_session_id=session.id).first()
        if existing:
            from app.services.continuity_service import capture_summary
        ...
        with self._lock:
            ...
            try:
                ...
                summary = ClinicalHistorySummary.objects.get(interview_session_id=session.id)
            from app.services.continuity_service import capture_summary
    ...
    def current(self, patient_id: str) -> ClinicalHistorySummary | None:
        ...
        return ClinicalHistorySummary.objects(interview_session_id=session.id).first() if session else None

    def list_all(self) -> list[ClinicalHistorySummary]:
        """Return a stable snapshot for the demo physician queue."""
        with self._lock:
            return list(ClinicalHistorySummary.objects.order_by("created_at"))

    def get_owned(self, summary_id: str, patient_id: str) -> ClinicalHistorySummary:
        summary = ClinicalHistorySummary.objects(pk=summary_id).first()
        if not summary:
    ...
    def update(
        ...
        with self._lock:
            ...
            if summary.status == "confirmed":
                if sections is not None and any(summary.sections.get(k) != v for k, v in sections.items()):
                    raise HTTPException(409, "Confirmed clinical history is read-only")
                return summary
            ...
            if patient_acknowledged and not summary.patient_acknowledged_at:
