...
import { getLabReport, lookupLabPatient, registerLabPatient, uploadLabReport, verifyLabPatient,
  type Demographics, type IdentifierType, type LabPatient, type LabReport } from '../services/lab';

type Stage = 'add' | 'verify' | 'report' | 'processing' | 'complete';
const emptyDetails: Demographics = { full_name: '', date_of_birth: '', gender: 'Prefer not to say', address: '', mobile: '', email: '' };
...
export default function LabWorkflow() {
  ...
  useEffect(() => { heading.current?.focus(); }, [stage, registration]);
  useEffect(() => {
    if (!reportId) return;
    let active = true;
  ...
  const reset = () => {
    ...
    setRegistration(false); setConfirmed(false); setConsented(false); setVerificationId(''); setError(''); setStage('add');
  };
  const select = (record: LabPatient) => {
    setPatient(record); setDetails({ full_name: record.full_name, date_of_birth: record.date_of_birth, gender: record.gender,
      address: record.address || '', mobile: record.mobile || '', email: record.email || '' });
    setConfirmed(false); setConsented(false); setVerificationId(''); setStage('verify');
  };
  const demographicFields = <>
    {(['full_name', 'date_of_birth', 'mobile', 'email', 'address'] as const).map(field => <label key={field}>{t(`lab.${field}`)}
      <input type={field === 'date_of_birth' ? 'date' : field === 'email' ? 'email' : field === 'mobile' ? 'tel' : 'text'}
        required={field === 'full_name' || field === 'date_of_birth'} value={details[field] || ''} disabled={busy}
        max={field === 'date_of_birth' ? new Date().toLocaleDateString('en-CA') : undefined}
        maxLength={field === 'address' ? 500 : field === 'full_name' ? 100 : 254}
  ...
  return <>
    <a className="skip-link" href="#lab-content">{t('skip.content')}</a>
      ...
      <div className="consent-layout lab-layout">
        <section className="card continuity-form lab-form" aria-busy={busy || (stage === 'processing' && !error)}>
          ...
          {stage === 'add' && !registration && <form onSubmit={async e => {
            ...
            try {
              const match = await lookupLabPatient(identifierType, identifier);
              if (version !== requestVersion.current) return;
              if (match.patient) select(match.patient);
              else { setDetails({ ...emptyDetails, ...(identifierType === 'phone' ? { mobile: identifier } : identifierType === 'email' ? { email: identifier } : {}) }); setRegistration(true); }
            } catch (reason) { setError(reason instanceof Error ? reason.message : t('patient.error')); } finally { setBusy(false); }
          }}>
            <label>{t('lab.identifierType')}<select disabled={busy} value={identifierType} onChange={e => setIdentifierType(e.target.value as IdentifierType)}>
          ...
          {stage === 'verify' && patient && <form onSubmit={async e => {
            e.preventDefault(); if (!confirmed || !consented || busy) return; setBusy(true); setError('');
            try { const result = await verifyLabPatient(patient.id, details, consented, confirmed); setPatient(result.patient); setVerificationId(result.verification_id); setStage('report'); }
            catch (reason) { setError(reason instanceof Error ? reason.message : t('patient.error')); } finally { setBusy(false); }
          }}>
          ...
          {stage === 'report' && patient && <>
            ...
            {busy && <ProgressIndicator stage="uploading" />}<DocumentFilePicker label={t('lab.upload')} disabled={busy} onFiles={async files => {
              ...
              try {
                ...
                setReport(result); setParams({ report: result.id }); setStage('processing');
              } catch (reason) { setError(reason instanceof Error ? reason.message : t('patient.error')); } finally { setBusy(false); }
            }}/><p>{t('lab.fileHelp')}</p>
