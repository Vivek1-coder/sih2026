...
export function ProfileDetailsTab({ profile }: { profile: ProfileDetails }) {
  ...
  return <dl className="profile-details-grid">{(['full_name', 'date_of_birth', 'gender', 'contact_number', 'address', 'abha_id', 'blood_group', 'allergies', 'emergency_contact', 'preferred_language'] as const).map(key => <div key={key}>
    <dt>{t(`profile.${key}`)}</dt><dd>{Array.isArray(profile[key]) ? profile[key].join(', ') || t('patient.empty') : key === 'date_of_birth' && profile[key] ? formatDate(profile[key]) : ['gender', 'preferred_language'].includes(key) ? ui(String(profile[key] || 'common:notProvided')) : profile[key] || t('patient.empty')}</dd>
  </div>)}</dl>;
}
...
export function MedicinesTab({ medicines }: { medicines: Medicine[] }) {
  ...
  return <>{!medicines.length && <p>{t('patient.empty')}</p>}{['active', 'completed'].map(status => <section key={status}>
    <h3>{t(`patient.${status}`)}</h3>{medicines.filter(m => m.status === status).map(m => <article className="history-row" key={m.id}>
      ...
      <p>{formatDate(m.start_date)} — {m.end_date ? formatDate(m.end_date) : t('patient.ongoing')}</p>
...
export function DocumentsTab({ documents }: { documents: ProfileDocument[] }) {
  ...
  return <>{error && <p role="alert">{errorText(error)}</p>}{!documents.length && <p>{t('patient.empty')}</p>}{documents.map(d => <article className="history-row" key={d.id}>
    <strong>{d.original_filename}</strong><p>{t(`document.${d.document_type}`)} · {t(`role.${d.uploaded_by_role}`)} {d.uploaded_by_id}</p>
    <p><time dateTime={d.uploaded_at}>{formatDate(d.uploaded_at, { dateStyle: "medium", timeStyle: "short" })}</time></p>
    <button className="button secondary" disabled={downloading !== null} onClick={async () => {
      if (downloading) return; setDownloading(d.id);
      try {
      ...
      } catch { setError('patient.error'); } finally { setDownloading(null); }
    }}>{downloading === d.id ? <Loader /> : t('patient.download')}</button>
  </article>)}</>;
}
...
export function SessionHistoryTab({ sessions, medicines }: { sessions: Visit[]; medicines: Medicine[] }) {
  ...
  return <>{!sessions.length && <p>{t('patient.empty')}</p>}{sessions.map(s => <article className="history-row" key={s.id}>
    <strong>{formatDate(s.created_at, { dateStyle: "medium", timeStyle: "short" })} · {s.location || t('patient.empty')}</strong>
    <p>{t(`patient.${s.status}`)} · {t('patient.doctor')}: {s.assigned_doctor_id || t('patient.pending')}</p>
    <button disabled={busy !== null} className="button secondary" onClick={async () => {
      if (busy) return; setBusy(s.id); setError('');
      try { setDetail(await patientRequest<Visit>(`/api/patient/sessions/${s.id}`)); }
      catch { setError('patient.error'); } finally { setBusy(null); }
    }}>{busy === s.id ? <Loader /> : t('patient.viewSession')}</button>
  </article>)}
    ...
    {detail && <section ref={detailRef} tabIndex={-1} className="card continuity-panel" aria-label={t('patient.session')}>
      <h3>{t('patient.answers')}</h3>{detail.answers.map(a => <div className="history-row" key={a.id}><strong>{a.question_text}</strong><p>{a.value}</p><small>{formatDate(a.answered_at, { dateStyle: "medium", timeStyle: "short" })}</small></div>)}
      ...
      <h3>{t('patient.prescriptions')}</h3>{detail.prescriptions?.map(p => <article className="history-row" key={p.id}>
        <p>{t('patient.doctor')}: {p.doctor_id} · {formatDate(p.issued_at, { dateStyle: "medium", timeStyle: "short" })}</p><p>{p.notes}</p>
...
export function MedicationSummaryTab({ summary }: { summary: MedicationSummary | null }) {
  ...
  return <><p>{formatDate(summary.generated_at, { dateStyle: "medium", timeStyle: "short" })} · {t(summary.confirmed ? 'patient.confirmed' : 'patient.draft')}</p>
...
export default function PatientProfilePage() {
  ...
  useEffect(() => {
    ...
    patientRequest<PatientProfile>('/api/patient/profile').then(p => { if (active) setData(p); })
      .catch(() => { if (active) setError('patient.error'); });
    return () => { active = false; };
  }, [retryCount]);
  return <PatientShell active="Identify">
    ...
    {error && <p className="form-error" role="alert">{errorText(error)}<button className="button secondary" onClick={() => setRetryCount(n => n + 1)}>{t("common:retry")}</button></p>}
    {!data && !error && <Skeleton />}
