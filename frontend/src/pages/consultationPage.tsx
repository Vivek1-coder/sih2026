...
export default function Consultation() {
  ...
  const persist = async (confirm: boolean) => {
    ...
    setMessage("");
    try {
      const updated = await updateSummary(consult.summary.id, {
        sections,
  ...
  return (
    <>
      ...
      <main className="consult-main">
        <button className="back-link" onClick={() => navigate("/physician")}>
          ...
          <>
            <div className="consult-head">
              <div className="table-patient">
                ...
                <div>
                  <span className="eyebrow">Consult-ready history</span>
                  <h1>{consult.queue.display_name}</h1>
                  <p>
                    {consult.queue.token} · {consult.queue.department} · version{" "}
                    {consult.summary.version}
                  </p>
                </div>
              </div>
            ...
            {consult.queue.red_flags.length > 0 && (
              <div className="flag-banner" role="alert">
                <AlertTriangle size={20} />
                <div>
                  <strong>Priority triage signal</strong>
                  <p>{consult.queue.red_flags.join(" · ")}</p>
                </div>
              </div>
            )}
            <div
              className="ai-review-note physician-summary-warning"
              role="note"
            >
              <Sparkles size={17} />
              <strong>AI-drafted history—not an autonomous diagnosis.</strong>
              <span>
                Compare every field with the patient and source documents before
                confirming.
              </span>
            </div>
            <div className="consult-grid summary-consult-grid">
              <aside className="card consult-timeline">
                <h2>Source record</h2>
                <h3>Interview</h3>
                <p className="muted">
                  {consult.interview.answers.length} answers ·{" "}
                ...
                {consult.documents.length === 0 ? (
                  ...
                  consult.documents.map((document) => (
                    <div className="consult-file" key={document.id}>
                      <FileText size={14} />
                ...
                <h3>ABDM demo push</h3>
                <p className="muted">
                  {consult.abdm?.bundle_id ?? "Not pushed"}
                </p>
              </aside>
              <section
                className="history-column physician-editable-summary"
                aria-label="Structured clinical history"
              >
                {Object.entries(sections).map(([title, value]) => (
                  <label className="card editable-history" key={title}>
              ...
              <aside className="card clinical-actions">
                <h2>Physician controls</h2>
                <p className="muted">
                  Saving keeps the draft editable. Confirmation records clinical
                  review.
                </p>
                <button
                ...
                >
                  <Check size={17} />{" "}
                  {confirmed ? "History confirmed" : "Confirm reviewed history"}
                </button>
                {message && (
