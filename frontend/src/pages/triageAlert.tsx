  ...
  useEffect(() => {
    ...
    getCurrentInterview()
      .then((session) => {
        if (active)
          setAlerts(
            session.alerts.filter((alert) => alert.priority === "urgent"),
          );
      })
      .catch(() => undefined);
    return () => {
  ...
  return (
    <PatientShell active="Converse">
      <main className="triage-alert-page" role="alert" aria-live="assertive">
        ...
        <p>
          {ui(
            "triageAlert:your_answers_contain_symptoms_that_need_prompt_review",
          )}
        </p>
        {alerts.map((alert) => (
          <div className="card triage-reason" key={alert.id}>
            ...
            <span>
              {ui("triageAlert:reported")}
              {alert.evidence.join(" · ")}
            </span>
          </div>
        ))}
        <div className="triage-actions">
          <a className="button primary" href="tel:112">
            <PhoneCall size={18} />
            {ui("triageAlert:call_112_if_this_is_an_emergency")}
          </a>
          <button
          ...
          >
            <ArrowLeft size={18} />
            {ui("triageAlert:return_to_answers")}
          </button>
        </div>
        <small>
          <ShieldCheck size={14} />
          {ui(
            "triageAlert:notify_onsite_clinical_staff_immediately_if_available",
          )}
        </small>
      </main>
    </PatientShell>
